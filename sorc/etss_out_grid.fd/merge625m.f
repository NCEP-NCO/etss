      SUBROUTINE merge625m (AREA,startH,endH)
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!c
!c    PURPOSE:
!c       MERGING OF THE MODEL GRIDDED DATA TO 625m ALASKA NDFD GRID BY USING
...
!c       08/2015--Huiqing Liu /MDL Updated to use tropical and extra-tropical 
!c                                 grids
!c       01/2016--Huiqing Liu /MDL Updated to merge 625m conus grid file
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      INTEGER         startH,endH
      CHARACTER*1     BASIN
      CHARACTER*255   FIL_11,FIL_12,FIL_49,FIL_9
      ...
      CHARACTER*255   FIL_41,FIL_42,FIL_43,FIL_44,FIL_45,FIL_46
      ...
      real,dimension(:,:),allocatable   :: Atmp,HaMax
      real,dimension(:,:,:),allocatable :: Hb,Hb_Tide
...
!c Maximum Surge 
      ...
      CALL GETENV('FORT9',FIL_9)
      OPEN(9,FILE=FIL_9,FORM='UNFORMATTED') 
...
!C***** TO LOCATE THE INDEX FOR BIG MAP AT FILL DATA*****
      ...
      IF (Nx == 1073) THEN !5/6km grid
          NGRIBM = 2958000
          Res_con = 5079406
          Res_ala = 5953125 
      ELSEIF (Nx == 8577) THEN !625m grid
...
!             READ(12+M) ((ATMP(I,J),I=1,IMXB(M)),J=1,JMXB(M))
        ...
        DO M=1,2
!c           write(*,*)imxb(m),jmxb(m)
!           READ(44+M) ((ATMP(I,J),I=1,IMXB(M)),J=1,JMXB(M))
           READ(44+M) ((HB_TIDE(M,I,J),I=1,IMXB(M)),J=1,JMXB(M))
        ENDDO
...
!c       READ TIDE ONLY RESULTS FOR 2 EXTRA-TROPICAL BASINS (EEX2 and EGM3)
        IF (FLEEXT=='stormtide') THEN
...
!C******* DO MAP PROJECTION ****************
     ...
     1                     HB_TIDE(1,IVAL(1,I,J),JVAL(1,I,J))
                 ELSE
                   HA(I,J)=HB(1,IVAL(1,I,J),JVAL(1,I,J))
                 ENDIF
     ...
     1                     HB_TIDE(2,IVAL(2,I,J),JVAL(2,I,J))
                 ELSE
                   HA(I,J)=HB(2,IVAL(2,I,J),JVAL(2,I,J))
                 ENDIF
               ENDIF
            ...
            elseif (mask(i,j)>100.and.mask(i,j)<10000) then ! 29 Tropical basins with 2 overlap
               ...
               if (hb(ibsn1,IVAL(ibsn1,I,J),JVAL(ibsn1,I,J))>80) then !one of them is dry then ignore this

            !      ha(i,j)=hb(ibsn2,IVAL(ibsn2,I,J),JVAL(ibsn2,I,J))
                  ha(i,j)=9999

               elseif (hb(ibsn2,IVAL(ibsn2,I,J),JVAL(ibsn2,I,J))>80) 
     $         then
     ...
     3                     (1-Perc(i,j)*.01) 
            ...
            elseif (mask(i,j)>10000) THEN ! 29 Tropical basins with 3 overlap
               ...
               if (hb(ibsn1,IVAL(ibsn1,I,J),JVAL(ibsn1,I,J))>80.and.
     1             hb(ibsn2,IVAL(ibsn2,I,J),JVAL(ibsn2,I,J))>80.and.
     2             hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))>80) then

                  ha(i,j)=9999

               elseif (hb(ibsn1,IVAL(ibsn1,I,J),JVAL(ibsn1,I,J))>80
     1           .and.hb(ibsn2,IVAL(ibsn2,I,J),JVAL(ibsn2,I,J))>80)then

                  ha(i,j)=hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))

               elseif (hb(ibsn1,IVAL(ibsn1,I,J),JVAL(ibsn1,I,J))>80
     1           .and.hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))>80)then
     ...
     1           .and.hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))>80)then
               ...
               elseif (hb(ibsn1,IVAL(ibsn1,I,J),JVAL(ibsn1,I,J))>80)then

                  ha(i,j)=hb(ibsn2,IVAL(ibsn2,I,J),JVAL(ibsn2,I,J))*0.5
     1                   +hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))*0.5

               elseif (hb(ibsn2,IVAL(ibsn2,I,J),JVAL(ibsn2,I,J))>80)then

                  ha(i,j)=hb(ibsn1,IVAL(ibsn1,I,J),JVAL(ibsn1,I,J))*0.5
     1                   +hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))*0.5

               elseif (hb(ibsn3,IVAL(ibsn3,I,J),JVAL(ibsn3,I,J))>80)then
...
!C***** WRITE OUT NEW MAP DATA
!c      WRITE(18) ((HA(I,J),J=1,NY),I=1,NX)
!C SET IFCSTHR TO THE CURRENT HOUR (THROUGH THE LOOP)
      IFCSTHR=N-1
!c      CALL MKGRIB(CGRIB,LCGRIB,RYEAR,RMONTH,RDAY,RHOUR,RMIN,RSEC,
!c     1            HA,NX,NY,AREA,IFCSTHR)
      if (N >= startH) then
