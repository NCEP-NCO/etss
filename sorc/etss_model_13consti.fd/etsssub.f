...
!           H. LIU   AUG 2017 MDL MODIFYING TO READ 13 km GFS WIND
!                    WCOSS MACHINE
!      PARAMETER (M$=600,N$=600,L$=5000,NCT$=2500,LC$=L$)
      include 'parm.for'
!      PARAMETER (M$=999,N$=999,L$=39999,NCT$=3000,LC$=L$)
!      COMMON /DUMB5/  ZB(M$,N$),ZBM(M$,N$)
!      COMMON /DUMB7/  UB(M$,N$),VB(M$,N$),HB(M$,N$)
!      COMMON /SCRTCH/ HSUB(M$,N$)
!      COMMON /DUMB10/ HMX(M$,N$)
!      COMMON /ELPDST/ ELPCT(M$),ELPDT(M$),ELPCL(M$),ELPDL(M$)
!      COMMON /ELPDS2/ ELPCL2(M$),ELPDL2(M$),SINL2(N$),SINT2(N$)
!      COMMON /PLRPST/ SINT(N$),COST(N$),SINL(N$),COSL(N$)
!      COMMON /SETMIX/ IS(N$),MS(N$),MF(N$)
!      COMMON /SETMX1/ IE(N$),ME(N$)
!      COMMON /ITREE/  ITREE(M$,N$)
!      COMMON /KTREE/  KTREE(L$)
!      CHARACTER*1     ITREE,KTREE
!      COMMON /KSPCON/ KSKP(M$,N$)
!      CHARACTER*1     KSKP
!      COMMON /DUMB18/ ISQR(L$),JSQR(L$),ISIDE(L$),HWEIR(L$),
!     1                ZBMIN(L$)
!      COMMON /ACT1D/  FLWSQR(L$),F1DACT(L$)
!      CHARACTER*1     F1DACT
!      COMMON /DUMCUT/ CUTL(NCT$),CUTLI(NCT$),CUTLE(NCT$)
C
!      COMMON /CS1DA/  COS1D(LC$),SIN1D(LC$)
!      COMMON /BANK2/  DELCUT(LC$),BANK(LC$,2)
!      COMMON /LLXFLE/YLT(M$,N$),YLG(M$,N$)
!     Increase to .5 by .5 Degree Huiqing.Liu /MDL April 2014
      include 'parm_gfs_new1hr.for'
!      PARAMETER (IMX$=71,JMX$=161)
      COMMON /DATINC/ XLAT1,XLON1,DELLT,DELLN,IMXC,JMXC
      COMMON /UVP/ XIU(IMX$,JMX$,2),XIV(IMX$,JMX$,2),XIP(IMX$,JMX$,2),
     1               XIUG(IMX$,JMX$,2),XIVG(IMX$,JMX$,2)
      COMMON /IUV/   IU(IMX$,JMX$),IV(IMX$,JMX$),IP(IMX$,JMX$),
     1               IUG(IMX$,JMX$),IVG(IMX$,JMX$)
      INTEGER*2    IU,IV,IP,IUG,IVG
C
...
! Huiqing.Liu /MDL ETSS2.0 June 2014
      ...
      CHARACTER*4 AIDENT
! Huiqing.Liu /MDL Aug. 2018      
!     RHOWG = WATER DENSITY*GRAVITY (MB/FT)
!     MB = 100 * Pa = 100 * N / m**2 = 100 * (kg * m/s**2) /m**2
C --------------------------------------------------------------------
...
C      JHR=61, IF IBGNT=JHR-48, KINPUT=3
      KINPUT=(IBGNT-1)/6+1
C      WRITE (*,*)'   JHR IBGNT KINPUT ', JHR,IBGNT,KINPUT
C      WRITE (*,*) 'ENTER FILENAME 1'
C      READ (*,'(A)') FILNAM
C      OPEN (33,FILE=FILNAM)
      KCNT=1
C     WRITE (*,*)'   KINPUT = ',KINPUT
!
!Huiqing.Liu AceInfo/MDL March-2018
!Read the interpolation table for GFS 13km
      DO I = 1, IMXB
      DO J = 1, JMXB
         READ (8,*) N_gfs(i,j), M_gfs(i,j)
      ENDDO
      ENDDO
      CLOSE(8)
! Skip to the right wind data information
!
       DO
         IF (KCNT.GT.KINPUT) EXIT
         READ (33) IMXC,JMXC,Ix1,Ix2,Iy1,Iy2
         READ (33) DUM1,DUM2
         WRITE (*,'(A)') DUM1
...
!      write(*,*)( 'IDATE=',IDATE1(i), i=1,4)
      call tflush
!
!
C
       CALL GEOWND
       IF (IGEO.EQ.1) CALL GEO2BL
C
C
      DO 200 J=1,JMXC
      DO 200 I=1,IMXC
...
C
      KINPUT=KINPUT+1
C     WRITE (*,*)'   KINPUT = ',KINPUT
      READ (33) IDUM,JMXC,Ix1,Ix2,Iy1,Iy2
      READ (33) DUM1,DUM2
C     WRITE (*,'(A)') DUM1
      READ (33) ((IU(I,J),I=1,IMXC),J=1,JMXC)
      READ (33) ((IV(I,J),I=1,IMXC),J=1,JMXC)
...
C
      ZZ=NDLTHR*INCW
      WGT=(ITIME+1-NHRW)/ZZ
C      WRITE (*,*)'   ITIME NHRW', ITIME,NHRW,INCW,WGT,CLAT,CLON
C
      NBNDRY=0
      CALL WDPINT(WGT,CLAT,CLON,NBNDRY,IGEO)
C
      RETURN
      END

C
      SUBROUTINE STMVL1(IGEO)
C        J. CHEN                  MAY 1992 MDL   IBM 360/195
C
C        PURPOSE
...
C           LOCAL INTERPOLATIONS IN TIME IS USED.
        ...
        CLON=0.
C
C      WRITE (*,*)' ITMADV-1',ITMADV-1,XLAT(ITMADV+INCW-1),
C     1     YLONG(ITMADV+INCW-1),CLAT,CLON
C
        KINPUT=KINPUT+1
C     WRITE (*,*)'   KINPUT = ',KINPUT
C     WRITE (*,*)' KINPUT INTV() ITIME',KINPUT,INTV(KINPUT),ITIME
C
C        KINPUT=11, FINISHED READING ANALYZED DATA
        IF (KINPUT.GE.11) THEN
C -------------------------------------------------------------------
C       WRITE (*,*) ' ENTER FILENAME 2 (UNIT=34)',KINPUT
C        WRITE (*,*) ' FORECAST WILL START 12 HOURS LATER.'
C        READ (*,'(A)') FILNAM
C        OPEN (34,FILE=FILNAM)
          READ (34) IDUM,JDUM,Ix1,Ix2,Iy1,Iy2
          READ (34) DUM1,DUM2
!      write(*,'(A)') DUM1
!      call tflush
...
!      call tflush
          ENDIF
C
C      FORECAST FIELDS, KINPUT > 11, ARE 3 HOURS APART.
!C      FORECAST FIELDS, KINPUT > 11, ARE 1 HOURS APART.
!          IF (KINPUT.GE.12) INCW=12.01/4.
          IF (KINPUT.GE.12) INCW=1
C-----------------------------------------------------------------------
        ELSE
C ----------------------------------------------------------------------
          INCW=6.01
...
C
      ...
      END
C
      SUBROUTINE WDPINT(WGT,CLAT,CLON,NBNDRY,IGEO)
C        J. CHEN               MAY 1992 MDL   IBM 360/195
C
C        PURPOSE
...
C
      ...
      XLT=YLT(I,J)
      XLN=YLG(I,J)
      CALL GRDINT(XLT,XLN,CLAT,CLON,WGT,OUT,XIP,I,J)
      PP(I,J)=OUT
 355  CONTINUE
      ELSE
C
...
C
      ...
      XLT=YLT(I,J)
      XLN=YLG(I,J)
      CALL GRDINT(XLT,XLN,CLAT,CLON,WGT,OUT,XIU,I,J)
...
C
      ...
      XLT=YLT(I,J)
      XLN=YLG(I,J)
      CALL GRDINT(XLT,XLN,CLAT,CLON,WGT,OUT,XIV,I,J)
      IF (IGEO.EQ.1.AND.(ITREE(I,J).EQ.'3'.OR.ITREE(I,J).EQ.'1'))
     1       OUT=OUT*.8
      WY(I,J)=OUT
 250  CONTINUE
...
C
      ...
      XLN=YLG(I,J)
      CALL GRDINT(XLT,XLN,CLAT,CLON,WGT,OUT,XIUG,I,J)
...
C
      ...
      CORR=2.*(7.292116E-5)*SIN(XLT*1.74532925E-2)
      XLN=YLG(I,J)
      CALL GRDINT(XLT,XLN,CLAT,CLON,WGT,OUT,XIVG,I,J)
...
C
      ...
      END
C
      SUBROUTINE GRDINT(XLATIN,YLONIN,CLAT,CLON,WGT,FLDOUT,FLD,ix,jx)
C        J. CHEN                 MAY 1992 MDL   IBM 360/195
C
C        PURPOSE
...
!                     WEST)
!                     NEED TO 360 + YLONIN TO CONVERT TO 0 ~ 360
!
!     XLAT1,XLON1   = LAT/LON OF NORTH/WEST CORNER OF THE INPUT WIND DATA,
!                     WHICH VALUE IS POSITIVE (IF LON IS WEST) 
!                     NEED TO 360 - XLON1 TO CONVERT TO 0 ~ 360
!c--------------------------------------------------------------------------
...
!      PARAMETER (IMX$=71,JMX$=161)
      COMMON /DATINC/ XLAT1,XLON1,DELLT,DELLN,IMXC,JMXC
       DIMENSION FLD(IMX$,JMX$,2)
       DIMENSION ZFLD(2)
!
! Added by Huiqing.Liu AceInfo/MDL March 2018
! For GFS table index (ix,jx)
! 
       integer ix,jx
!-----------------------------------------------------------
...
C
      ...
      IF (YLON.LE.0) YLON=360+YLON ! SLOSH grid longitude west is negative

!      DO I=1,JMXC ! JMXC longitude
!         IF (YLON >= Lonu(Ix1+I-1) .AND. YLON <= Lonu(Ix1+I))THEN
!            N = I
!            EXIT
!         ENDIF
!      ENDDO
!      DO J=1,IMXC ! IMXC latitude
!         IF (XLAT <= Latu(Iy1+J-1) .AND. XLAT >= Latu(Iy1+J))THEN
!            M = J
!            EXIT 
!         ENDIF          
!      ENDDO
...
C
      IF (M.LT.1.OR.N.LT.1.OR.M.GT.IMXC.OR.N.GT.JMXC) THEN
         WRITE (*,*)'  LAT/LON',XLAT,YLON,XLATIN,YLONIN
         WRITE (*,*) '  GRID POINT OUT OF BOUND ',M,N
         stop
      ELSE
         if (0.eq.1) then ! First interpolation (Bi-linear)
           W1=(Lonu(Ix1+N)-YLON)*(Latu(Iy1+M)-XLAT)
           W2=(YLON-Lonu(Ix1+N-1))*(Latu(Iy1+M)-XLAT)
           W3=(Lonu(Ix1+N)-YLON)*(XLAT-Latu(Iy1+M-1))
           W4=(YLON-Lonu(Ix1+N-1))*(XLAT-Latu(Iy1+M-1))
           WW=(Lonu(Ix1+N)-Lonu(Ix1+N-1))*(Latu(Iy1+M)-Latu(Iy1+M-1))

           ZFLD(K)=(W1*FLD(M,N,K) + W2*FLD(M+1,N,K)
     1             +W3*FLD(M,N+1,K) + W4*FLD(M+1,N+1,K))/WW
         else ! Second interpolation (Reverse of distance as weighting average)
           W1 = distance(Latu(Iy1+M-1),XLAT,Lonu(Ix1+N-1),YLON)
           W2 = distance(Latu(Iy1+M),XLAT,Lonu(Ix1+N-1),YLON)
           W3 = distance(Latu(Iy1+M-1),XLAT,Lonu(Ix1+N),YLON)
           W4 = distance(Latu(Iy1+M),XLAT,Lonu(Ix1+N),YLON)
           if ((W1*W2*W3*W4).ne.0)then
              ZFLD(K) = (FLD(M,N,K)/W1 + FLD(M+1,N,K)/W2 + 
     1                  FLD(M,N+1,K)/W3 + FLD(M+1,N+1,K)/W4)/
     2                  (1/W1 + 1/W2 + 1/W3 + 1/W4)
           else
             if (W1.eq.0)ZFLD(K) = FLD(M,N,K)
             if (W2.eq.0)ZFLD(K) = FLD(M+1,N,K)
             if (W3.eq.0)ZFLD(K) = FLD(M,N+1,K)
             if (W4.eq.0)ZFLD(K) = FLD(M+1,N+1,K)
           endif
         endif
C   
      ENDIF
...
!        Z1=COS((XLAT1-(I-1)*res_wnd)*1.74532925E-2)
        ...
        ZZ=SIN(Latu(Iy1+I-1)*1.74532925E-2)
        Z1=COS(Latu(Iy1+I-1)*1.74532925E-2)


        DO 201 J=2,JMXC-1
!        XVG=.25*(ZP(I+1,J+1)-ZP(I+1,J-1)+ZP(I-1,J+1)-ZP(I-1,J-1))
!        XUG=.25*(ZP(I+1,J+1)-ZP(I-1,J+1)+ZP(I+1,J-1)-ZP(I-1,J-1))
        XVG=.5*(ZP(I+1,J+1)-ZP(I+1,J-1)+ZP(I-1,J+1)-ZP(I-1,J-1))
     $ /abs((Lonu(Ix1+J)-Lonu(Ix1+J-2)))
        XUG=.5*(ZP(I+1,J+1)-ZP(I-1,J+1)+ZP(I+1,J-1)-ZP(I-1,J-1))
     $ /abs((Latu(Iy1+I)-Latu(Iy1+I-2)))
...
!        CORFL=CNSTL/SIN((XLAT1-(I-1)*res_wnd)*1.74532925E-2)
        ...
        CORF=CNST/SIN(Latu(Iy1+I-1)*1.74532925E-2)
        CORFL=CNSTL/SIN(Latu(Iy1+I-1)*1.74532925E-2)
...
!
     ...
     1 FIL_34,FIL_81,FIL_22,FIL_52,FIL_53,FIL_84,FIL_15,FIL_55,FIL_87
     2 ,FIL_50,FIL_51
!
...
!C    GFS grid lat and lon file
      call getenv('FORT8',FIL_11)
      open(8,file=FIL_11)

      call getenv('FORT9',FIL_11)
      open(9,file=FIL_11)

      call getenv('FORT10',FIL_11)
      open(10,file=FIL_11)
!C   Read GFS grid lat and lon
      do i = 1, 3072 ! GFS wind dimension in x direction
         read(9,*) Lonu(i) ! Data is from W to E (0 ~360) inner loop
      enddo
      close(9)
      do i = 1, 1536 ! GFS wind dimension in y direction
         read(10,*) Latu(i)
         Latu(i) = Latu(i) * -1 ! Data is from N to S (90 ~ -90) outerloop
      enddo
      close(10)
...
!         open current cycle water initial water condtion file
...
C        ITMS=96 MODEL SET 96 HOURS FORECAST FIELDS EVERY 3 HR.
      IBGS=48
      ITMS=96
      JHR=61
...
C 
c      READ (21,'(2I3,4X,2F5.2)',END=2000) IBGS,ITMS,SEADTM,DTMLAK
! Added by H.Liu/MDL Aug. 2018 for initial water condition
! Reading the basin name to detmine which water level value need (GOM/East)
      READ(21,*) IBGS,ITMS
      close(21)
      close(19)
      if (Iopt == 1) then ! Using one anomaly for whole est,gom from NHC
...
!
       CALL DEPSFC
! Added by Huiqing.Liu /AceInfo June 2019
! if idep=1 using all boundary points as the nesting boundary
! Other are using only deep water boundary points as the nesting boundary 
! The value of ITIDE is setted from slosh2.c. 
! Currently, we only use deep water points as the nesting boundary 
! Therefore, we set idep=0 
       idep=ITIDE 
       idep=0
...
C N.GT.94 ---- Spinup 48 hrs and N.GT.118 ----- Spinup 60 hrs
C------------------------------------------------------------------------------
!       IF (N.GT.94) THEN
