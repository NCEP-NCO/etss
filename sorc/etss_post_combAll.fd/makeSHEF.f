...
c-----------------------------------------------------------------------
      ...
      SUBROUTINE MAKESHEF(SSARR,TIARR,ANARR,NWSLI,OBSB,TIDB,DATE,U)
...
C       No obs, no anomaly, just tide and surge
          DO II = 1,FcstH+1
             ...
             END IF
          END DO
          DATTYP = 'HMIFX' ! Surge + Tide only; not bias-adjusted
...
C       Tide, surge, and anomaly are available
          DO II = 1,FcstH+1
             ...
             END IF
          END DO
          DATTYP = 'HMIFV' ! Surge + Tide + Anom; bias-adjusted
        END IF
      ELSE
C     No tide, just surge; Likely an AHPS gauge
        DO II = 1,FcstH+1
          ...
          END IF
        END DO
        DATTYP = 'HMIFW' ! Surge only; no total water level
      END IF
